...
contract UniswapTest is Test {
  ...
  function setUp() public {
    ...
    //. demonstrate vm.etch
    WETH w = new WETH();
    bytes memory runtime_code = address(w).code;
    vm.etch(weth, runtime_code);
  }

  //. `forge test --match-test test_solved_add_liquidity_good`
  //. -vv for printing console log
  //. -vvvv for printing trace
  function test_solved_add_liquidity_good() public {
    //. you can also use vm.prank(alice)
    vm.startPrank(alice); // msg.sender == alice
    ...
    vm.stopPrank(); // until this point
  }

  // `forge test --match-test test_solved_add_liquidity_fail`
  function test_solved_add_liquidity_fail() public {
    vm.startPrank(alice);
